...
class LocationEmbeddingModel(nn.Module):
    """
    論文に準じた単一の Location ID を利用する埋め込み層。
    0: <PAD>
    40001: <MASK>
    """
    ...
    def __init__(self, embed_size):
        ...
        self.location_embedding = nn.Embedding(
            num_embeddings=40002,
            embedding_dim=embed_size,
        )

    def forward(self, location_id):
        return self.location_embedding(location_id)
...
class EmbeddingLayer(nn.Module):
    def __init__(self, embed_size):
        ...
        self.location_embedding = LocationEmbeddingModel(embed_size)
        self.timedelta_embedding = TimedeltaEmbeddingModel(embed_size)

    def forward(self, day, time, location, timedelta):
        day_embed = self.day_embedding(day)
        time_embed = self.time_embedding(time)
        location_embed = self.location_embedding(location)
        timedelta_embed = self.timedelta_embedding(timedelta)

        embed = day_embed + time_embed + location_embed + timedelta_embed
        return embed
...
class LPBERT(nn.Module):
    ...
    def forward(self, day, time, location, timedelta, len):
        embed = self.embedding_layer(day, time, location, timedelta)
