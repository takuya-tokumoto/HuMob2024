...
class HuMobDatasetTaskBTrain(Dataset):
    def __init__(self, path):
        ...
        for uid, traj in tqdm(self.df.groupby("uid")):
            ...
            if len(d_unique[(d_unique >= np.min(d_unique) + 15) & (d_unique <= np.max(d_unique) - 14)]) == 0:
                continue
            mask_d_start = np.random.choice(
                d_unique[(d_unique >= np.min(d_unique) + 15) & (d_unique <= np.max(d_unique) - 14)]
            )
            ...
            # 201でMASKしたd以下のみ学習データへ採用 -> MASKしたd以降のデータは考慮しないように
            selected_indices = np.where(d <= mask_d_end)

            self.d_array.append(d[selected_indices] + 1)
            self.t_array.append(t[selected_indices] + 1)
            self.input_x_array.append(input_x[selected_indices])
            self.input_y_array.append(input_y[selected_indices])
            self.time_delta_array.append(time_delta[selected_indices])
            self.label_x_array.append(label_x[selected_indices] - 1)
            self.label_y_array.append(label_y[selected_indices] - 1)
            self.len_array.append(len(d[selected_indices]))
