...
def collate_fn(batch):
    ...
    input_padded = pad_sequence(input, batch_first=True, padding_value=0)
    ...
    label_padded = pad_sequence(label, batch_first=True, padding_value=0)
...
def compute_validation_metrics(args, load_model_path):
    ...
    with torch.no_grad():
        for data in tqdm(dataloader_val):
            ...
            output = load_model(data["d"], data["t"], data["input"], data["time_delta"], data["len"])
            ...
            for step in range(len(output)):
                ...
                flat_idx = torch.argmax(output[step], dim=-1)
                pred_x = flat_idx // 200  # 0-indexed
                pred_y = flat_idx % 200  # 0-indexed

                # pre_x, pre_y = pred[-1][0].item(), pred[-1][1].item()
                pred.append(torch.tensor([pred_x, pred_y]))

            pred = torch.stack(pred)
            generated = (
                torch.cat(
                    (
                        data["d"][pred_mask].cpu().unsqueeze(-1) - 1,
                        data["t"][pred_mask].cpu().unsqueeze(-1) - 1,
                        pred + 1,
                    ),
                    dim=-1,
                )
            ...
            label_masked = label[pred_mask]
            ref_list = []
            for i in range(label_masked.shape[0]):
                ref_val = label_masked[i].item()  # 0-indexed
                ref_x = (ref_val // 200) + 1
                ref_y = (ref_val % 200) + 1
                ref_list.append((ref_x, ref_y))
            ...
            reference = (
                torch.cat(
                    (
                        ...
                        # label[pred_mask] + 1,
                        torch.tensor(ref_list, device=device),
                    ),
                    dim=-1,
                )
                .cpu()
...
def taskA(args):
    ...
    for epoch_id in range(args.epochs):
        for batch_id, batch in enumerate(tqdm(taskA_dataloader_train)):
            ...
            output = model(batch["d"], batch["t"], batch["input"], batch["time_delta"], batch["len"])
            # label = torch.stack((batch["label_x"], batch["label_y"]), dim=-1)
            label = batch["label"]

            pred_mask = batch["input"] == 40001
